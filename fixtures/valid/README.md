...
| `single-repo-minimal.json` | One repo; `svc:` and `sql:` scopes alongside `mongo:` and `{repo}:`; a `service` parent; a band-skipping `call` (api → data_access, skipping `domain`) |
...
| `annotated-edge.json` | An `annotated` edge with `source: null` and `source_count: 0`; a `topic` node; non-ASCII labels and tags for the escaping and NFC rules |
