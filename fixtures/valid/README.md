...
| `unknown-dangling-refs.json` | Two repos; two dangling references drawn to synthetic `unknown` nodes (parser §4.2): an `http` ref from Swift whose id `unknown:http//notes/{id}/archive` carries the path value verbatim after the first `/`, and a `symbol` ref from Python. Each `unknown` node is `inferred` with a reason naming the ref and its origin, has `sources: []`, and inherits its caller's tier |
