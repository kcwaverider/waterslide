...
| `unknown-dangling-refs.json` | Two repos; two dangling references drawn to synthetic `unknown` nodes (parser §4.2): an `http` ref from Swift whose id `unknown:http:%2Fnotes%2F{id}%2Farchive` percent-encodes the path value, and a `symbol` ref from Python. Each `unknown` node is `inferred` with a reason naming the ref and its origin, has `sources: []`, a label of the form `unresolved http /notes/{id}/archive`, and inherits its caller's tier |
| `shared-branch-ordinal.json` | `derived-ids.json` with its fork rewritten as one limb making two calls: both fork edges carry `branch_ordinal: 0` and the same condition, to different targets, with ids re-derived. Ordinals number alternatives, not edges (invariant 15); the fork edge-id key includes `to`, so the ids stay distinct |
